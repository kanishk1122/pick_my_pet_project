...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.petName.length < 3) throw new Error("Pet name must be at least 3 characters long");
      if (formData.description.length < 20) throw new Error("Description must be at least 20 characters long");
      if (!selectedAddress) throw new Error("Please select an address");
      if (selectedImages.length === 0) throw new Error("Please upload at least one image");

      const phoneRegex = /(?:\+?\d{1,3}[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}/g;
      const linkRegex = /(https?:\/\/[^\s]+)/g;

      if (phoneRegex.test(formData.petName) || linkRegex.test(formData.petName)) {
      ...
      if (phoneRegex.test(formData.description) || linkRegex.test(formData.description)) {
        throw new Error("Description cannot contain phone numbers or links.");
      }

      if (formData.ageValue && (isNaN(formData.ageValue) || formData.ageValue < 0)) {
      ...
      if (result.meta.requestStatus === "fulfilled") {
        ...
        const errorMessage = result.payload?.message || "An unexpected error occurred during post creation.";
      ...
      if (error.message === "Network Error" || error.code === "ECONNABORTED") {
        errorMessage = "The upload timed out. Please try with smaller or fewer images.";
      } else if (error.response?.status === 413) {
        errorMessage = "The images are too large. Please use smaller images or fewer images.";
      } else {
  ...
  const speciesAndBreedSection = (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      <div>
        ...
        {isLoadingSpecies && (
          <div className="mt-1 text-xs font-bold text-stone-400">Loading species...</div>
        )}
      </div>
      ...
      >
        ...
        {isLoadingBreeds && (
          <div className="mt-1 text-xs font-bold text-stone-400">Loading breeds...</div>
        )}
        {availableBreeds.length === 0 && formData.species && !isLoadingBreeds && (
    ...
    >
      ...
      <div className="w-full bg-white rounded-[2.5rem] border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6 md:p-10">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-stone-500 font-medium">Share your pet with the world and find them a loving home.</p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-10 bg-stone-50 rounded-3xl p-6 border-2 border-stone-100 border-dashed">
            <p className="text-stone-700 mb-4 font-bold text-lg">
              Upload Pet Images <span className="text-stone-400 text-sm font-normal">(Max 5)</span>
            </p>
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              {selectedImages.length < 5 && (
                <label className="aspect-square border-2 border-dashed border-emerald-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-emerald-50 hover:border-emerald-500 transition-all group">
                  ...
                  <span className="text-emerald-600 font-bold text-sm">Add Photo</span>
                </label>
              )}
            </div>
          </div>
          ...
          <div className="mb-10 pt-8 border-t-2 border-stone-100">
            ...
            {addresses && addresses.length > 0 ? (
              ...
              <div className="text-center py-8 bg-stone-50 rounded-2xl border-2 border-dashed border-stone-200">
                <p className="text-stone-500 font-medium mb-4">No addresses found.</p>
              </div>
            )}
          ...
          <div className="flex justify-end pt-6 border-t-2 border-stone-100">
            <button
              ...
              className={`brand-button-accent w-full md:w-auto text-lg ${
                isSubmitting ? "opacity-50 cursor-not-allowed shadow-none translate-y-[2px]" : ""
              }`}
            >
